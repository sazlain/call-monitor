...
public class CallHistoryController {
    ...
    public ResponseEntity<Map<String, Object>> history(
        ...
        List<String> roles = jwtUtil.extractRoles(auth.substring(7));
        ...
        if (isAdmin) {
            ...
            // Agente — solo sus extensiones
            AgentEntity agent = agentRepo.findByUserId(userId).orElse(null);
        ...
        List<CallHistoryResponse> content = pageResult.getContent().stream()
                .map(c -> {
                    ...
                    // Datos del agente
                    AgentEntity agent = agentRepo.findByExtension(c.getCallerExtension())
                            .orElse(null);
                    if (agent == null) {
                        // Intentar normalizar extensión
                        String rawExt = c.getCallerExtension();
                        if (rawExt != null && rawExt.length() > 4) {
                    ...
                    return CallHistoryResponse.builder()
                            .id(c.getId())
                            .callId(c.getCallId())
                            .callerExtension(c.getCallerExtension())
                            ...
                            .agentName(agentName)
                            .agentExtension(c.getCallerExtension())
